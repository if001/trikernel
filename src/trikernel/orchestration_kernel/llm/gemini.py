...
def newGeiminiClient(
    config: Optional[GeminiConfig] = None,
    model: Optional[str] = None,
    timeout: int = 60,
):
    config = config or load_gemini_config()
    model = model or config.model
    timeout = timeout
    _logger = get_logger("trikernel.gemini")
...
# class GeminiLLM(OrchestrationLLM):
#     def __init__(
#         self,
#         config: Optional[GeminiConfig] = None,
#         model: Optional[str] = None,
...
#
#     def bind_tools(self, tools: Sequence[BaseTool]) -> "GeminiLLM":
