...
from .tools.state_tools import build_state_tools, build_task_tools
...
def register_default_tools(kernel: ToolKernel) -> None:
    tools: Iterable[BaseTool] = (
        build_state_tools() + build_system_tools() + build_writing_tools()
        # + build_file_tools()
...
def register_deep_agent_tools(kernel: ToolKernel) -> None:
    tools: Iterable[BaseTool] = build_task_tools()
    for tool in tools:
        kernel.tool_register(tool)
